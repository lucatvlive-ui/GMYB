...
function loadTrainingPlanFromDB() {
    const uid = sessionStorage.getItem('userUID');
    if (!uid || !db) {
        console.log('Nicht angemeldet oder Firebase nicht verfügbar');
        return;
    }
    ...
    .then(doc => {
        if (doc.exists && doc.data().trainingPlan) {
            ...
            displayTrainingPlan();
            console.log('Trainingsplan geladen:', trainingPlan);
        } else {
            console.log('Kein Trainingsplan gefunden - leere Liste');
            trainingPlan = [];
            displayTrainingPlan();
        }
    })
