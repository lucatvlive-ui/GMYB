...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (!currentPage.includes('Login.html')) {
        ...
        // Lade Trainingsplan wenn auf Trainingsplan.html
        if (currentPage.includes('Trainingsplan.html')) {
            setTimeout(() => {
                console.log('🔄 Trainingsplan.html geladen - lädt Daten...');
                loadTrainingPlanFromDB();
            }, 1000);
        }
    }
...
function savePlan() {
    ...
    .then(() => {
        console.log('✅ Plan gespeichert:', trainingPlan);
...
function loadTrainingPlanFromDB() {
    const uid = sessionStorage.getItem('userUID');
    
    // Versuche ZUERST lokal zu laden (schneller)
    const localPlan = localStorage.getItem('trainingPlan');
    if (localPlan) {
        try {
            trainingPlan = JSON.parse(localPlan);
            displayTrainingPlan();
            console.log('✅ Plan aus localStorage geladen:', trainingPlan);
        } catch (e) {
            console.log('Fehler beim Parsen des lokalen Plans');
        }
    }
    
    // Dann von Firebase laden (Sync)
    if (!uid || !db) {
        console.log('⚠️ Nicht angemeldet - nur lokaler Plan verfügbar');
        return;
    }
    ...
    .then(doc => {
        if (doc.exists && doc.data().trainingPlan) {
            trainingPlan = doc.data().trainingPlan;
            displayTrainingPlan();
            localStorage.setItem('trainingPlan', JSON.stringify(trainingPlan));
            console.log('✅ Plan von Firebase geladen:', trainingPlan);
        } else {
            console.log('❌ Kein Plan in Firebase gefunden');
        }
    })
    .catch(error => {
        console.error('❌ Fehler beim Laden von Firebase:', error);
    });
}
